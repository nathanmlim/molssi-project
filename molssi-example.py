...
class NCMCMove(BaseIntegratorMove):
    ...
    def getMasses(self, atom_subset, topology):
        ...
        masses = unit.Quantity(
            np.zeros([int(len(atoms)), 1], np.float32), unit.dalton)
    ...
    def rotate(self, context, thermodynamic_state, sampler_state):
        ...
        masses, totalmass = self.getMasses(atom_subset,
                                           thermodynamic_state.topology)
        ...
        rot_move = np.dot(reduced_pos, rand_rotation_matrix
                          ) * sub_positions.unit + center_of_mass
    ...
    def apply(self,
        ...
        for attempt_counter in range(self.n_restart_attempts + 1):
            ...
            if alch:
                # If we reassign velocities, we can ignore the ones in sampler_state.
                sampler_state.apply_to_context(
                    context, ignore_velocities=False)

            # Subclasses may implement _before_integration().
            simulation = self._before_integration(context, integrator,
                                                  thermodynamic_state)
            ...
            try:
                ...
                for n in range(self.n_steps + 1):
                    ...
                    if alch:
                        ...
                        alch_lambda = integrator.getGlobalVariableByName(
                            'lambda')
                        lambda_step = integrator.getGlobalVariableByName(
                            'lambda_step')
                        protocol_work = integrator.getGlobalVariableByName(
                            'protocol_work')

                        thermodynamic_state.set_alchemical_variable(
                            'lambda', alch_lambda)
                        ...
                        if n == rotation_step:
                            context = self.rotate(context, thermodynamic_state,
                                                  sampler_state)
                    ...
                    if not alch:
                        if n % reporter.reportInterval == 0:
                            print('MD Step:', n, 'PE:',
                                  context_state.getPotentialEnergy())
                            reporter.report(simulation, context_state)
                    else:
                        if n % reporter.reportInterval == 0:
                            print('NCMC---IntgrParameters:', 'Step:',
                                  step, 'PE:',
                                  context_state.getPotentialEnergy(), 'Work:',
                                  protocol_work, 'Lambda: ', alch_lambda,
            ...
            if restart:
                ...
                if attempt_counter == self.n_restart_attempts - 1:
                    ...
                    if not alch:
                        sampler_state.apply_to_context(
                            context,
                            ignore_velocities=self.reassign_velocities)
                    thermodynamic_state.apply_to_context(context)
    ...
    def _after_integration(self, context, thermodynamic_state):
        ...
        try:
            ...
            n_accepted = integrator.getGlobalVariable(
                ghmc_global_variables['naccept'])
            n_proposed = integrator.getGlobalVariable(
...
class NCMCSampler(object):
    ...
    def _computeAlchemicalCorrection(self, md_context, md_init_context_state,
                                     ncmc_init_context_state,
        ...
        md_context = self.setContextFromState(
            md_context, ncmc_final_context_state, velocities=False)
        ...
        correction_factor = (
            ncmc_init_PE - md_init_PE + alch_PE - ncmc_final_PE) * (-1.0 / kT)

        md_context = self.setContextFromState(md_context,
                                              md_final_context_state)
    ...
    def update(self):
        ...
        md_context, md_init_context_state, md_final_context_state = self.move.apply(
        ...
        alch_integrator = self._get_alch_integrator(
            self.alch_thermodynamic_state)
        ncmc_context, ncmc_init_context_state, ncmc_final_context_state = self.move.apply(
            self.alch_thermodynamic_state,
            self.sampler_state,
            alch_integrator,
            self.reporter,
        ...
        work_ncmc = ncmc_context._integrator.getLogAcceptanceProbability(
            ncmc_context)
        ...
        if not np.isnan(work_ncmc):
            correction_factor = self._computeAlchemicalCorrection(
                md_context, md_init_context_state, ncmc_init_context_state,
                ncmc_final_context_state, ncmc_context._integrator.kT)
            logger.debug(
                'NCMCLogAcceptanceProbability = %.6f + Alchemical Correction = %.6f'
        ...
        if work_ncmc > randnum:
            ...
            if not math.isclose(
                    md_init_context_state.getPotentialEnergy()._value,
                    md_PE._value,
                    rel_tol=float('1e-%s' % rtol)):
                logger.error(
        ...
        if self.verbose:
            final_energy = md_context.getState(
...
alch_thermodynamic_state = ThermodynamicState(
    system=toluene_alchemical_system, temperature=temperature)
